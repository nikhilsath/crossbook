...
| `load_field_schema()`                         | Queries the database for all defined fields (table `field_schema`) and loads the `FIELD_SCHEMA` global. It builds a nested dict of `{table: {field: {"type": ..., "options": [...], "foreign_key": ..., "layout": {...}}}}` for use in templates and validation. This is called once at startup to initialize the schema info. |
