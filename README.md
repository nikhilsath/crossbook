...
│   │   ├── autosize_text.js         # Fit text inside detail-view cells
...
* **Frontend Interaction:** Dynamic behaviors powered by JavaScript modules in `static/js/`:
  ...
  * `autosize_text.js` to auto-fit text whenever fields resize
...
- **`FIELD_SCHEMA`:** Retrieved via `get_field_schema()` whenever needed. It returns a dictionary in the form `{table: {field: {"type": ..., "options": [...], "foreign_key": ..., "layout": {...}}}}` describing how each field should be treated in the UI.
