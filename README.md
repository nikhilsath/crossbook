...
- **Title and Identifier:** At the top of the left panel, the record’s primary name is displayed in a large heading. The field marked as the title in the table’s schema supplies this value. If no field is flagged, the app falls back to a column named after the table or the first available column. Next to it, the record’s ID is shown for reference.
